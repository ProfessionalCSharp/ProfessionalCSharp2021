...
public class GamesFactory
{
    ...
    public Game CreateGame(string gameType, string playerName)
    {

        static string GetRandomValue(string[] items) => items[Random.Shared.Next(items.Length)];

        static Game<ColorField, SimpleColorResult> Get6x4SimpleGame(string gameType, string playerName) =>
        ...
        static Game<ColorField, ColorResult> Get6x4Game(string gameType, string playerName) =>
            new(Guid.NewGuid(), gameType, playerName, 4, 12)
            {
                Codes = Enumerable.Range(0, 4).Select(i => new ColorField(GetRandomValue(s_colors6))).ToArray()
            };

        static Game<ColorField, ColorResult> Get8x5Game(string gameType, string playerName) =>
        ...
        static Game<ShapeAndColorField, ShapeAndColorResult> Get5x5x4Game(string gameType, string playerName) =>
