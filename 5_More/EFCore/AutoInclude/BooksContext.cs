...
public class BooksContext(DbContextOptions<BooksContext> options) : DbContext(options)
{
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        bookBuilder.Navigation(b => b.Author).AutoInclude();
        bookBuilder.Navigation(b => b.Chapters).AutoInclude();
    ...
    private Book[] GetSampleBooks()
        => [.. Enumerable.Range(1, 100)
            .Select(id => 
            new Book($"title {id}", "sample", id) 
            { 
                Publisher = _publishers[Random.Shared.Next(0, 2)], 
                AuthorId = Random.Shared.Next(1, 3) 
            })];

    private Chapter[] GetSampleChapters() =>
        [.. Enumerable.Range(1, 100)
        .Select(id => 
            new Chapter($"chapter {id % 10}", id) { BookId = (id / 10 + 1) })];
