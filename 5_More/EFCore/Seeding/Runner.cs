...
class Runner(MenusContext menusContext)
{
    ...
    public async Task SingleOrDefaultAsync(string text)
    {
        ...
        MenuItem? menu = await _menusContext
            .MenuItems
            .TagWith("SingleOrDefault")
            .SingleOrDefaultAsync(m => m.Text == text);
    ...
    public async Task FirstOrDefaultAsync(string title)
    {
        ...
        MenuItem? menu = await _menusContext
            .MenuItems
            .TagWith("FirstOrDefault")
            .FirstOrDefaultAsync(m => m.Text == title);
    ...
    {
        IAsyncEnumerable<MenuItem> menuItems = _menusContext.MenuItems
            .AsAsyncEnumerable();
    ...
    public async Task RawSqlAsync(string term)
    {
        ...
        var menuItems = await _menusContext.MenuItems
            .FromSqlInterpolated($"SELECT * FROM [mc].[MenuItems] WHERE TEXT = '{term}'")
            .TagWith("RawSQL")
            .ToListAsync();
    ...
    public async Task UseCompiledQueryAsync()
    {
        ...
        await foreach (var menuItem in _menusContext
            .MenuItemsByTextAsync("menu 26"))
        {
            Console.WriteLine(menuItem);
        }
        Console.WriteLine();
    ...
    public async Task UseEFunctions(string textSegment)
    {
        ...
        var menuItems = await _menusContext.MenuItems
            .Where(m => EF.Functions.Like(m.Text, likeExpression))
            .ToListAsync();
    ...
    public async Task DeleteDatabaseAsync()
    {
        ...
        if (input?.ToLower() == "y")
        {
            bool deleted = await _menusContext.Database
                .EnsureDeletedAsync();
