...
namespace PipesReader
{
    public class Program
    {
        ...
        {
            using AnonymousPipeServerStream reader = new(PipeDirection.In, HandleInheritability.Inheritable);
        ...
        private static void PipesReader2(string pipeName)
        {
            try
            {
                NamedPipeServerStream pipeReader = new(pipeName, PipeDirection.In);
                using StreamReader reader = new(pipeReader);
        ...
        private static void PipesReader(string pipeName)
        {
            try
            {
                using NamedPipeServerStream pipeReader = new NamedPipeServerStream(pipeName, PipeDirection.In);
